...
import { 
  ...
  CheckCircle2, Circle, Square, Triangle, ShieldAlert, Wifi, Info, Laptop, MapPin, Copy, Check, Download, MessageSquare, Phone, Search, Users, TrendingUp, Calendar, Edit2, Save
} from 'lucide-react';
...
export default function App() {
  ...
  const [pendingErrorLocation, setPendingErrorLocation] = useState<PathPoint | null>(null);
  ...
  const saveStudentEdit = () => {
    ...
    // Aggiorniamo anche l'avatar se il nome cambia per coerenza
    const avatarUrl = `https://api.dicebear.com/7.x/initials/svg?seed=${editName}&backgroundColor=3b82f6`;
    const updated = students.map(s => 
      s.id === selectedStudent.id 
        ? { ...s, name: editName, phoneNumber: editPhone, avatarUrl } 
        : s
    );
    setStudents(updated);
    setSelectedStudent({ ...selectedStudent, name: editName, phoneNumber: editPhone, avatarUrl });
    setShowEditStudent(false);
  };

  const logErrorWithNote = () => {
    const timestamp = Date.now();
    const location = pendingErrorLocation || lastPosRef.current || { lat: 0, lng: 0, timestamp };
    const newError: ErrorEvent = {
      id: Math.random().toString(36).substr(2, 9),
      location,
      note: errorNote.trim() || "Intervento critico istruttore",
      timestamp
    };
  ...
  const handleFinish = async () => {
    ...
    // Resettiamo punteggi per la prossima volta
    setScores({});
    setCurrentPath([]);
    setCurrentErrors([]);
  };
          ...
          <div className="w-full max-w-sm grid grid-cols-1 gap-4 animate-in slide-in-from-bottom duration-500 delay-300">
            ...
            >
              ...
              <div className="text-left">
                <h3 className="text-xl font-black uppercase leading-none">Database Allievi</h3>
                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1">Lista completa</p>
              </div>
            </button>
          </div>
        </div>
            ...
            <div className="bg-white rounded-t-[4rem] sm:rounded-[4rem] p-12 w-full max-w-md animate-in slide-in-from-bottom duration-500">
              ...
              <div className="space-y-6 mb-10 text-black">
                <input 
                  ...
                  className="w-full p-5 bg-gray-50 border-2 border-transparent rounded-[2rem] font-bold outline-none focus:border-blue-500 text-lg"
                />
                <input 
                  type="tel" 
                  ...
                  className="w-full p-5 bg-gray-50 border-2 border-transparent rounded-[2rem] font-bold outline-none focus:border-green-500 text-lg"
                />
              </div>
              <div className="flex gap-4">
                ...
                <button onClick={() => {
                  ...
                  setShowAddStudent(false); setSelectedStudent(n); setView('DASHBOARD');
                }} className="flex-2 px-10 py-5 bg-blue-600 text-white rounded-[2rem] font-black shadow-2xl uppercase tracking-widest text-xs">Salva</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col safe-top">
        <header className="bg-white p-6 border-b flex items-center justify-between shadow-sm sticky top-0 z-50">
          ...
          <div className="text-center">
            ...
            <p className="text-[9px] font-black uppercase text-gray-400 mt-1">Scegli un allievo</p>
          </div>
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col safe-top">
        <header className="bg-white p-6 border-b flex items-center justify-between shadow-sm sticky top-0 z-50">
          ...
          <div className="text-center flex-1 mx-4">
            ...
            <p className="text-[9px] font-black uppercase text-blue-600 mt-1">Dati Personali</p>
          </div>
          <button onClick={() => {
            setEditName(selectedStudent.name);
            setEditPhone(selectedStudent.phoneNumber || '');
        ...
        <main className="p-6 space-y-6 max-w-2xl mx-auto w-full pb-32">
          <button onClick={() => {setView('EVALUATION'); startTracking();}} className="w-full py-12 bg-blue-600 text-white rounded-[3.5rem] shadow-2xl flex flex-col items-center gap-4 active:scale-95 transition-all">
          ...
          <div className="bg-white p-10 rounded-[3.5rem] border shadow-sm grid grid-cols-2 gap-4">
            <div className="text-center border-r-2 border-gray-50">
              ...
              <p className="text-[10px] font-black uppercase text-gray-400 mt-2 tracking-widest">SESSIONI</p>
            </div>
            <div className="text-center">
          ...
          <div className="space-y-4">
            <h4 className="text-[11px] font-black uppercase text-gray-400 px-6 tracking-[0.2em]">STORICO LEZIONI</h4>
            {studentHistory.length === 0 ? (
              <div className="bg-white p-10 rounded-3xl border border-dashed border-gray-200 text-center text-gray-300 text-xs font-bold uppercase italic">Ancora nessuna guida effettuata</div>
            ) : studentHistory.map((res, i) => (
              <button key={i} onClick={() => {setSelectedResult(res); setView('HISTORY_DETAIL');}} className="w-full bg-white p-6 rounded-[2.5rem] border border-gray-100 flex items-center justify-between active:bg-gray-50 shadow-sm transition-all">
                <div className="text-left">
                  <p className="text-base font-black text-blue-600">{new Date(res.date).toLocaleDateString()}</p>
                  <p className="text-[11px] text-gray-400 font-bold uppercase truncate max-w-[200px] mt-1">{res.feedback || "Report disponibile"}</p>
                </div>
        ...
        {showEditStudent && (
          <div className="fixed inset-0 bg-black/80 z-[200] flex items-end sm:items-center justify-center p-4 backdrop-blur-md">
            <div className="bg-white rounded-t-[4rem] sm:rounded-[4rem] p-10 w-full max-w-md animate-in slide-in-from-bottom duration-500">
              <h3 className="text-2xl font-black mb-8 uppercase tracking-tighter text-black">Modifica Profilo</h3>
              <div className="space-y-4 mb-8 text-black">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl font-bold outline-none focus:border-blue-500"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Telefono WhatsApp</label>
                  <input 
                    type="tel" value={editPhone} onChange={e => setEditPhone(e.target.value)}
                    className="w-full p-4 bg-gray-50 border-2 border-transparent rounded-2xl font-bold outline-none focus:border-green-500"
                  />
                </div>
              </div>
              <div className="flex gap-4">
                <button onClick={() => setShowEditStudent(false)} className="flex-1 py-4 font-black text-gray-400 uppercase tracking-widest text-xs">Annulla</button>
                <button onClick={saveStudentEdit} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-lg uppercase tracking-widest text-xs">Salva</button>
              </div>
            </div>
          </div>
        )}
    ...
    return (
      <div className={`min-h-screen flex flex-col safe-top transition-colors duration-500 ${isFlashing ? 'bg-red-100' : 'bg-gray-50'}`}>
        <header className="p-6 border-b flex items-center justify-between sticky top-0 bg-white z-50 shadow-sm">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center gap-1 justify-center">
              ...
              <p className="text-[9px] font-black uppercase text-green-600 tracking-widest">Valutazione Live</p>
            </div>
          </div>
          <button onClick={handleFinish} className="bg-blue-600 text-white px-8 py-4 rounded-[1.5rem] font-black text-xs uppercase shadow-xl active:scale-95">CONCLUDI</button>
        </header>
        ...
        <div className="fixed bottom-12 left-0 w-full flex justify-center z-[100] px-8">
          <button 
            onClick={() => {
              const location = lastPosRef.current || { lat: 0, lng: 0, timestamp: Date.now() };
              setPendingErrorLocation(location);
              setIsFlashing(true);
              setTimeout(() => setIsFlashing(false), 500);
              setShowErrorModal(true);
              if (navigator.vibrate) navigator.vibrate([300]);
            }}
            className="w-full py-14 bg-red-600 text-white rounded-[4rem] shadow-2xl border-[12px] border-white active:scale-90 transition-all flex items-center justify-center gap-6"
          >
        ...
        {showErrorModal && (
          <div className="fixed inset-0 bg-black/90 z-[300] flex items-center justify-center p-6 backdrop-blur-xl">
            <div className="bg-white rounded-[4rem] p-10 w-full max-w-sm animate-in zoom-in duration-300">
              <div className="w-16 h-16 bg-red-100 text-red-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm"><AlertCircle size={32}/></div>
              <h3 className="text-2xl font-black text-center mb-6 text-black uppercase tracking-tight">Cosa è successo?</h3>
              <textarea 
                autoFocus
                value={errorNote}
                onChange={e => setErrorNote(e.target.value)}
                placeholder="Es. Mancata precedenza, invasione corsia, stop ignorato..."
                className="w-full p-6 bg-gray-50 border-2 border-gray-100 rounded-3xl font-bold outline-none focus:border-red-500 text-black mb-6 min-h-[140px] text-lg"
              />
              <div className="flex gap-4">
                <button onClick={() => setShowErrorModal(false)} className="flex-1 py-4 font-black text-gray-400 uppercase tracking-widest text-xs">Indietro</button>
                <button onClick={logErrorWithNote} className="flex-2 py-4 bg-red-600 text-white rounded-2xl font-black shadow-lg uppercase tracking-widest text-xs">Conferma Log</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col safe-top">
        <header className="bg-white p-6 border-b flex items-center justify-between sticky top-0 z-[100] shadow-sm">
          ...
          <div className="flex gap-3">
            <button onClick={() => sendWhatsApp(selectedResult)} className="p-4 bg-green-500 text-white rounded-2xl shadow-xl active:scale-90 transition-all"><MessageSquare size={24}/></button>
            <button onClick={() => {
        ...
        <main className="p-6 space-y-8 max-w-2xl mx-auto w-full pb-32 overflow-y-auto">
          <div className="bg-white p-10 rounded-[3.5rem] border shadow-sm border-l-[16px] border-l-blue-600">
             <div className="flex items-center gap-2 mb-3">
               <TrendingUp size={16} className="text-blue-600" />
               <p className="text-[10px] font-black uppercase text-blue-600 tracking-widest">Analisi Metodo DEC</p>
             </div>
             <p className="text-lg text-gray-700 leading-relaxed italic font-medium">"{selectedResult.feedback}"</p>
          </div>
          
          <div className="space-y-4">
            <h4 className="text-[12px] font-black uppercase text-gray-400 px-6 tracking-[0.3em]">MAPPA DEL PERCORSO</h4>
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-6 safe-top safe-bottom">
        <div className="bg-white w-full max-w-md rounded-[5rem] shadow-2xl overflow-hidden text-center p-14 space-y-10 animate-in zoom-in duration-500">
          <div className="w-28 h-28 bg-blue-600 text-white rounded-full flex items-center justify-center mx-auto shadow-2xl scale-110"><CheckCircle2 size={56}/></div>
          <h2 className="text-4xl font-black uppercase tracking-tighter text-black leading-none">REPORT PRONTO</h2>
          <div className="bg-gray-50 p-10 rounded-[3.5rem] border text-lg italic text-gray-600 leading-relaxed font-medium shadow-inner">
            {isGenerating ? (
              <div className="flex flex-col items-center gap-4 py-4">
                ...
                <p className="font-black text-[11px] uppercase tracking-widest text-blue-600">L'IA sta elaborando...</p>
              </div>
            ) : aiFeedback}
          </div>
          <button onClick={() => setView('DASHBOARD')} className="w-full py-8 bg-black text-white rounded-[2.5rem] font-black text-sm uppercase tracking-[0.3em] active:scale-95 shadow-2xl">Vedi Dashboard</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center text-blue-600 p-8 text-center">
      <div className="w-14 h-14 border-4 border-current border-t-transparent rounded-full loader-spin mb-8 shadow-2xl"></div>
      <p className="text-[11px] font-black uppercase tracking-[0.4em] animate-pulse">Caricamento Metodo DEC...</p>
    </div>
  );
}
